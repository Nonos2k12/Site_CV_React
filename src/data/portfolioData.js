export const portfolioData = [
  {
    id: 1,
    name: "Instagram clone",
    languages: ["tous", "javascript", "css", "react"],
    languagesIcons: ["fab fa-js", "fab fa-css3-alt", "fab fa-react"],
    source: "https://github.com/Nonos2k12/instagram_clone",
    info: "This project uses React (custom hooks, useContext, useState, useEffect, useRef), Firebase (Firestore/auth) and Tailwind CSS. This React project has multiple pages: login, sign up, dashboard (to view/like/comment on photos), and user profiles. The sign-in page connects to Firebase when a user tries to sign in, and when a user signs up, Firebase auth is used to store the user in the Firebase auth database.",
    picture: "./media/project1.png",
  },
  {
    id: 2,
    name: "Social Network (Twitter like)",
    languages: ["tous", "javascript", "css", "react"],
    languagesIcons: ["fab fa-js", "fab fa-css3-alt", "fab fa-react"],
    source: "https://github.com/Nonos2k12/Projet-Doranco-RS",
    info: "This project uses React, MongoDB, Express, NodeJS, Redux, JWT, Mongoose, Bcrypt and Sass. This React project has multiple pages: login, sign up, dashboard, home and user profiles. The sign-in page connects to a MongoDB database when a user tries to sign in, and when a user signs up. A JSON Web Token is used to identify the user in the MongoDB database.",
    picture: "./media/project2.png",
  },
  {
    id: 3,
    name: "PHP Calendar",
    languages: ["tous", "php", "css"],
    languagesIcons: ["fab fa-php", "fab fa-css3-alt"],
    source: "https://github.com/Nonos2k12/calendrier_php_sql",
    info: "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Accusamus laboriosam aut nihil? Possimus, id facilis dignissimos repudiandae modi dolorum ea accusantium, veritatis debitis odio eveniet quos incidunt eaque nemo eius perferendis consequatur? Lorem ipsum, dolor sit amet consectetur adipisicing elit. Accusamus laboriosam aut nihil? Possimus, id facilis dignissimos repudiandae modi dolorum ea accusantium.",
    picture: "./media/project3.png",
  },
  {
    id: 4,
    name: "3D Bell-Viewer",
    languages: ["tous", "javascript", "css"],
    languagesIcons: ["fab fa-js", "fab fa-css3-alt"],
    source: "https://github.com/Nonos2k12/Babylon-JS-3D-Bell-Viewer",
    info: "This is one of my internship project. Our client wanted a 3D representation of an old bell in the city of Auxerre. This project uses the BabylonJS library which allows to create 3D objects using Javascript. ",
    picture: "./media/project4.png",
  },
  {
    id: 5,
    name: "Space Invaders",
    languages: ["tous", "javascript", "css"],
    languagesIcons: ["fab fa-js", "fab fa-css3-alt"],
    source: "https://github.com/Nonos2k12/Space-invaders",
    info: "A simple space invaders game using Javascript and CSS.",
    picture: "./media/project5.png",
  },
  {
    id: 6,
    name: "Projet Vivino",
    languages: ["tous", "javascript", "vue", "css", "php"],
    languagesIcons: [
      "fab fa-php",
      "fab fa-js",
      "fab fa-vuejs",
      "fab fa-css3-alt",
    ],
    source: "https://github.com/Nonos2k12/interface-vue-php",
    info: "This project is a dynamic interface to find the best wines according to a country or grape variety. It uses PHP for the backend, VueJS for the frontend and Sass for the style. It's a single page application connect to a MySQL database.",
    picture: "./media/project6.png",
  },
];
